...
const CampaignExchangeDetails = () => {
  const { state } = useLocation();
  const { getDonations, contract, address, handleSell, handlePass, handleCancelPass } = useStateContext();
  ...
  console.log("checkkkk exchange", state)
  const fetchSellPackage = async () => {
    setIsLoading(true)
    await handleSell(state.campaign.campaignId, state.campaign.target)
    ...
    <div>
      ...
      <div className="mt-[60px] flex lg:flex-row flex-col gap-5">
              ...
              <div className="mt-[20px] flex flex-col gap-4">
                {donators.length > 0 ? donators.map((item, index) => (
                  <div key={`${item.donator}-${index}`} className="flex justify-between items-center gap-4">
                    <p className="font-epilogue font-normal text-[16px] text-[#b2b3bd] leading-[26px] break-ll">{address}</p>
        ...
        <div className="flex-1">
          <h4 className="font-epilogue font-semibold text-[18px] text-white uppercase">Exchange</h4>

          <div className="mt-[20px] flex flex-col p-4 bg-[#1c1c24] rounded-[10px]">
            <p className="font-epilogue fount-medium text-[20px] leading-[30px] text-center text-[#808191]">
              Exchange a Health Package
            </p>
            <div className="mt-[30px]">
              <input
                type="number"
                placeholder={state.campaign?.target}
                step="0.01"
                disabled={true}
              ...
              <div className="my-[20px] p-4 bg-[#13131a] rounded-[10px]">
                <h4 className="font-epilogue font-semibold text-[14px] leading-[22px] text-white">Wishing you a bright and joyful day!</h4>
                <p className="mt-[20px] font-epilogue font-normal leading-[22px] text-[#808191]">Would you like to explore a Mental Health Care Package? Your journey to well-being starts here.</p>
              </div>

              {state.sellerAddress !== address ?
                  <CustomButton
                  disabled={amount !== state.campaign?.target}
                  btnType="button"
                  title={"ExChange a Health Package"}
                  styles="w-full bg-[#8c6dfd]"
                  handleClick={fetchSellPackage}
              /> :
                  <CustomButton
                      disabled={amount !== state.campaign?.target}
                      btnType="button"
...
export default CampaignExchangeDetails
